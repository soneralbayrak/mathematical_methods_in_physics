...
\bea
\w=&\w_{i_1\dots i_p}dx^{i_1}\wedge\dots\wedge dx^{i_p}\\
d\w=&\frac{\partial \w_{i_1\dots i_p}}{\partial x^k}dx^k\wedge dx^{i_1}\wedge\dots\wedge dx^{i_p}
\eea
...
\begin{enumerate}
	...
	&\quad\to \left((x_1,\dots,x_d)\to \pdr{v^i(x_1,\dots,x_d)}{ x^i}\right)\quad(\text{in Cartesian coordinates\footnotemark})
	\ee 
	\footnotetext{
...
\draftnote{
\begin{enumerate}
	\item We discussed the above points in class.
	\item We briefly discussed the spherical, cyclindrical, and polar coordinates. We derived the area and volume elements in these coordinates.
\end{enumerate}
}